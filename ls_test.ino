...
void displayXFrame() {
  ...
  static unsigned long lastFrame = 0;
  unsigned long now = micros();
  if (sensorCol == 1 && sensorRow == 0 && calcTimeDelta(now, lastFrame) >= 2000000) {
    lastFrame = now;
...
void displaySurfaceScanTime() { 
  ...
  if (sensorCol == 1 && sensorRow == 0) {
    ...
    ++scanCount;
    if (calcTimeDelta(now, lastFrame) >= 5000000 && scanCount > 0) {
      lastFrame = now;
...
void debugFreeRam() {
  static unsigned long lastFrame = 0;
  unsigned long now = micros();
  if (Device.serialMode && sensorCol == 1 && sensorRow == 0 && calcTimeDelta(now, lastFrame) >= 5000000) {
    lastFrame = now;
