...
reduce Global.mainNotes and Global.accentNotes from [12] to [9] and change from int to short? 
  96 bytes becomes 36 bytes, saves 60 bytes, saves 420 bytes with 6 presets
...
pack Global.switchBothSplits and Global.ccForSwitchCC65 into 1 byte, saves 35 bytes

pack Global.switchAssignment (3 bits) and Global.customSwitchAssignment (5 bits) into 1 byte, switchAssignments, saves 70 bytes
limits total # of functions to 32, 7 directly accessible, 25 custom functions that need scrolling
8ve down = 000xxxxx, 8ve up = 001xxxxx... auto 8ve = 110xxxxx, tap tempo = 11100111, legato = 11101000...
xxxxx stores the current custom function (7-31), so a single tap can get it back, important on stage
inline byte switchAssignment(byte s) {
  if (switchAssignments[s] < B11100000) return switchAssignments[s] >> 5;
  return switchAssignments[s] && B00011111;
}
inline void setSwitchAssignment(byte s, byte b) {
  if (b < 7) switchAssignments[s] = (switchAssignments[s] & B00011111) | (b << 5);
  else       switchAssignments[s] = b | B11100000;
} 

pack struct SequencerPattern, save 3 bytes = 12 bytes in struct SequencerProject, 12x17 = 204 bytes

some settings on the Global display are actually device settings, not global settings
  these settings are not song-dependent: serialMode, sleepDelay, etc.
  likewise, move some microLinnSplit or microLinnGlobal settings to microLinnDevice, reduces memory use 7:1
  split: defaultLayout, groupingCC
...
maybe CC faders special mode: make vertical faders, slightly glitchy because of the horizontal separator strips
  ...
  could also have 2 vertical 4-pad sliders per column
  either way, use alternating colors as before
  if (sensorCol == 25) preSendControlChange(side, 41, 16 * sensorRow + (sensorCell->currentCalibratedY / 16.0), true);
...
LS_CONST byte MICROLINN_SCALES[MICROLINN_ARRAY_SIZE] = {
  ...
  B0001110,  // M7
  B0000000,  //
  // 20edo   2L4s(4:3), 6L1s(3:2), 4L4s(3:2), 8L4s(2:1)
  B1111111,  // P1
  B0000000,  //
  B0001100,  //
  B0000010,  //
  ...
  B0000010,  //
  B0001000,  //
  // 21edo   5L3s(3:2), 3L6s(3:2), 1L9s(3:2), 9L3s(2:1)
  B1111111,  // P1
  B0000000,  //  
  B0001110,  //
  B0000001,  // P2
  ...
  B0001111,  //
  B0000000,  //
  // 22edo   1L6s(4:3), 6L2s(3:2), 4L5s(3:2), 10L2s(2:1)
  B1111111,  // P1
  B0000000,  // m2
  B0001010,  //
  B0000101,  //
...
byte microLinnLastClipLaunched = 0;                             // 0 = none, 1-8 = the 8 left-split clips, 9-16 = the 8 right-split clips
...
inline signed char microLinnSign (short num) {
  if (num > 0) return 1;
  if (num < 0) return -1;
  return 0;
}

inline int microLinnMod (int num, int base) {          // -13 % 10 = -3, but microLinnMod (-13, 10) = 7
  num %= base;
  if (num < 0) num += base;
  return num;
}
...
inline short microLinnLCM (short x,  short y) {        // returns the least common multiple of x & y
  float z = microLinnGCD (x, y);
  return (z == 0 ? 0 : abs (x * y) / z);        // LCM (x, 0) = x, LCM (0, y) = y, LCM (0, 0) = 0
}

// unused, delete?
inline short microLinnRound (float x) {
  return floor (x + 0.5);                       // round towards positive infinity
}

/************** lookup functions called only from ls_microLinn.ino ************************/

struct __attribute__ ((packed)) MicroLinn2sides {   // packed byte
  byte left:4;
  byte right:4;
};

//MicroLinn2sides note[128];
...
inline short microLinnTriIndex(byte edo, short edostep) {     // index into a triangular array
  edostep = microLinnMod (edostep, edo);
  return (edo * (edo - 1)) / 2 + edostep - 10;         // -10 to skip over edos 1-4
}

inline byte microLinnDots(byte edo, short fret) {
  return Device.microLinn.fretboards[microLinnTriIndex(edo, fret)];
}

inline byte microLinnRainbowColor(byte edo, short edostep) {
...
inline boolean microLinnIsScaleStep(byte edo, short edostep) {
  return microLinnIsScaleStep(edo, edostep, Global.activeNotes);
}

inline boolean microLinnIsScaleStep(byte edo, short edostep, byte scale) {
...
inline boolean isLinn200() {          // only call this function for text displays! not calibration, scan order, Z-bias, etc.!
//return false;                // uncomment this line to test linn128 text displays on a linn200
  return LINNMODEL == 200;
}

inline boolean isMicroLinnOn() {
  return Global.microLinn.EDO >= 5;
}

inline boolean isMicroLinnXfixOn() {
...
inline boolean isMicroLinnZmaxOn() {
...
inline boolean isMicroLinnImportingOn() {
  return microLinnImportingOn;
}

// called by scalePitch() in ls_midi.ino
inline boolean isMicroLinnColOffsetOn(byte side) {
...
inline short getMicroLinnVirtualEdostep(byte side, byte col, byte row) {
...
inline signed char getMicroLinnMidiNote(byte side, signed char note, byte group) {
  if (note < 0) return -1;
...
inline signed char getMicroLinnMidiNote(byte side, short edostep) {
  if (edostep < 0) return -1;
...
inline short getMicroLinnTuningBend(byte side, signed char note, byte group) {
...
inline short getMicroLinnTuningBend(byte side, short edostep) {
...
inline signed char rechannelMicroLinnGroup(byte side, signed char chan, byte group) {
...
inline boolean isMicroLinnDupeChannel(byte chan) {                  // chan runs 1-16
  boolean isUsed;
...
inline byte getMicroLinnSequencerBendRange() {
  if (edo >= 12) return 50;
  return ceil(600.0 / edo);                               // larger edos have a smaller bend range
}

inline void setMidiChannelSelect() {
...
inline void microLinnDeleteHammeredNote (byte i) {
  microLinnNumHammerOns -= 1;
...
void initializeMicroLinn() {
  ...
  Global.microLinn.locatingCC1 = 255;
  Global.microLinn.locatingCC2 = 255;
...
//memset(&Global.microLinn.largeEDOoffset, 0, sizeof(Global.microLinn.largeEDOoffset)); uncomment later
  ...
  for (byte side = 0; side < NUMSPLITS; ++side) {
    ...
    Split[side].microLinn.setHammerOnMode(0);
    Split[side].microLinn.setPullOffMode(0);
    ...
    Split[side].microLinn.groupingCC = 255;
...
inline void storeMicroLinnRainbowAndFretboard() {         // called from storeSettingsToPreset()
...
inline void loadMicroLinnRainbowAndFretboard() {          // called from loadSettingsFromPreset()
  byte rawEDO = Global.microLinn.EDO;
  if (rawEDO == 4) return;
  short i = microLinnTriIndex(rawEDO, 0);
...
void microLinnSetupKitesPersonalPrefs() {          // speed up debugging cycle, delete later once things are more stable
  ...
  for (byte p = 4; p <= 5; ++p) {
    ...
    for (byte side = 0; side < NUMSPLITS; ++side) {
      ...
      config.preset[p].split[side].microLinn.setHammerOnMode(0);       //  3 = R+L
    ...
    config.preset[p].split[RIGHT].microLinn.ccForLowRowW = 15;
...
void setupMicroLinn() {                              // runs when the Linnstrument powers up or a preset is loaded
  ...
  microLinnPerSplitXenSettingsLinked[4] = microLinnIsLinked(Split[LEFT].microLinn.groupingCC, Split[RIGHT].microLinn.groupingCC, 255);
...
inline void calcMicroLinnGlobalRowOffsetCents() {
...
inline void microLinnCalcGuitarRowOffsetCents(byte row) {
...
inline void calcMicroLinnNumChannels(byte side) {
  microLinnChannelCount[side] = 0;
  for (byte ch = 0; ch < 16; ++ch) {
    if (Split[side].midiChanSet[ch]) microLinnChannelCount[side] += 1;
  }
...
inline byte microLinnGetSplitWidth(byte side) {
  if (Global.splitActive) {
...
inline short microLinnTransposition(byte side) {                                  // # of edosteps to transpose by
  signed char direction = isLeftHandedSplit(side) ? -1 : 1;
...
#ifdef DEBUG_ENABLED
  midiSendControlChange(41, 41, 1, true);
  unsigned long calcTuningTimer = millis();
#endif
...
#ifdef DEBUG_ENABLED
  calcTuningTimer = calcTimeDelta(millis(), calcTuningTimer);
  midiSendControlChange(41, calcTuningTimer, 1, true);
#endif
}
...
inline short microLinnScaleStepsToEdoSteps(short scaleSteps, short startingStep) {
...
inline byte microLinnWarningColor (byte side) {
  return (Split[side].colorMain == COLOR_RED ? COLOR_MAGENTA : COLOR_RED);
}

inline void paintMicroLinnPlusMinus() {                                   // for the "8VE±" footswitch
  if (isLinn200()) {
...
inline void paintMicroLinnMinusPlus() {                                   // for the "8VE-+" footswitch
  if (isLinn200()) {
...
inline byte getMicroLinnRowOffsetColor(short rowOffset) {                            // for most Global displays
  for (byte side = 0; side < NUMSPLITS; ++side) {
...
inline byte getMicroLinnRowOffsetColorGuitar3(byte row) {                               // for microtonal guitar tuning display, col 1
...
inline byte getMicroLinnRowOffsetColorGuitar4(byte row) {                               // for microtonal guitar tuning display, numbers
  if (row == 0) return globalColor;
  if (getMicroLinnRowOffsetColor(Global.microLinn.guitarTuning[row]) == COLOR_RED) return COLOR_RED;
  return globalColor;
}

inline byte getMicroLinnGUIcolor() {                                                   // for custom row offset of -17 = -GUI
...
inline void enterMicroLinnConfig() {                             // called from Global Settings, col 1 row 0 or col 17 row 1
...
inline byte microLinnGetColor (boolean selected) {
  return selected ? COLOR_CYAN : COLOR_GREEN;
}

inline void microLinn_draw_string (byte col, byte row, const char* str, byte color, boolean erase) {
  if (isLinn200()) smallfont_draw_string(col, row, str, color, erase);
  else              condfont_draw_string(col, row, str, color, erase);
}

inline void microLinn_draw_string (byte col, byte row, const char* str, const char* str2, byte color, boolean erase) {
  if (isLinn200()) smallfont_draw_string(col, row, str,  color, erase);
...
inline void microLinnPaintConfigButtons() {
...
inline boolean microLinnIsConfigButton() {
...
inline boolean microLinnIsDisabledConfigButton() {                // a button that doesn't work because we're not in an edo
...
inline void microLinnPaintShowSplitSelection(boolean linked) {
...
inline void microLinnHandleShowSplit(boolean& linked) {   
  // adds the ability to affect both splits at once, modelled after handleOctaveTransposeNewTouch()
  // call this *after* the line "byte side = Global.currentPerSplit;", or else it works backwards
...
inline void microLinnPaintPerSplitSettingsButtons() {
...
void microLinnHandlePerSplitSettingsNewTouch() {
  ...
  switch (microLinnPerSplitSettingsRowNum) {
    case 7:
      ...
      break;
    case 6: {
      byte hammerOnMode = Split[side].microLinn.hammerOnMode();
      handleNumericDataNewTouchCol(hammerOnMode, 0, 3, true);
      Split[side].microLinn.setHammerOnMode(hammerOnMode);
      if (microLinnPerSplitSettingsLinked[6])
        Split[otherSide].microLinn.setHammerOnMode(hammerOnMode);
      break;
    }
    case 5:
      if (isMicroLinnOn()) {
...
inline void microLinnHandlePerSplitSettingsHold() {
...
inline void microLinnHandlePerSplitSettingsRelease() {                                // short-press col 1
  if (sensorCol == 1 && sensorRow >= 3) updateDisplay();
}

inline void microLinnPaintGlobalSettingsButtons() {
...
inline boolean microLinnIsLinnCC(byte CC) {
  // locating and grouping CCs should avoid CCs that the LinnStrument uses for other purposes
...
void microLinnPaintGlobalSettings() {
  ...
  if (microLinnGlobalSettingsNowScrolling) {
    switch (microLinnGlobalSettingsRowNum) {
      ...
      case 4: microLinnScrollSmall("LOCATING CC #1"); break;
      case 3: microLinnScrollSmall("LOCATING CC #2"); break;
    }
    microLinnPaintConfigButtons();
    microLinnPaintGlobalSettingsButtons();
  }

  byte locatingColor = globalColor;
  boolean matchingCCs = Global.microLinn.locatingCC1 == Global.microLinn.locatingCC2 &&
                        Global.microLinn.locatingCC2 != 255;                                  // locatingCC2 is always 255 on a Linn128
  switch (microLinnGlobalSettingsRowNum) {
    case 7:
    ...
    case 5:
      ...
      break;
    case 4:
      if (Global.microLinn.locatingCC1 == 255) {
        smallfont_draw_string(2, 1, "OFF", globalColor, false);
      } else {
        if (matchingCCs || microLinnIsLinnCC(Global.microLinn.locatingCC1)) locatingColor = COLOR_RED;
        paintNumericDataDisplay(locatingColor, Global.microLinn.locatingCC1, 0, false);
      }
      break;
    case 3:
      if (Global.microLinn.locatingCC2 == 255) {
        smallfont_draw_string(2, 1, "OFF", globalColor, false);
      } else {
        if (matchingCCs || microLinnIsLinnCC(Global.microLinn.locatingCC2)) locatingColor = COLOR_RED;
        paintNumericDataDisplay(locatingColor, Global.microLinn.locatingCC2, 0, false);
      }
      break;
  }
}
...
void microLinnHandleGlobalSettingsNewTouch() {
  ...
  switch (microLinnGlobalSettingsRowNum) {
    ...
    case 5:
      ...
      break;
    case 4:
      CC = Global.microLinn.locatingCC1;
      if (CC == 255) CC = -1;
      handleNumericDataNewTouchCol(CC, -1, 127, true);
      Global.microLinn.locatingCC1 = CC == -1 ? 255 : CC;                    // 255 means OFF
      updateDisplay();
      break;
    case 3:
      CC = Global.microLinn.locatingCC2;
      if (CC == 255) CC = -1;
      handleNumericDataNewTouchCol(CC, -1, 127, true);
      Global.microLinn.locatingCC2 = CC == -1 ? 255 : CC;
      updateDisplay();
      break;
  }
  updateDisplay(); 
...
inline void microLinnHandleGlobalSettingsRelease() {                                // short-press col 1
  byte row = isLinn200() ? 3 : 4;
  if (sensorCol == 1 && sensorRow >= row) updateDisplay(); 
}

inline void microLinnPaintEDO() {
  ...
  switch (microLinnPerSplitXenSettingsRowNum) {
    ...
    case 5:
      if ((Split[side].microLinn.tuningTable == 1 && microLinnNumGroups[side] > 1) ||               // in ON mode, but more than 128 notes?
          (Split[side].microLinn.tuningTable == 2 && Split[side].microLinn.groupingCC == 255) ||    // in CC mode, but no CC chosen?
      ...
      break;
    case 4:
      if (Split[side].microLinn.groupingCC == 255) {
        if (Split[side].microLinn.tuningTable == 2) {
          color = microLinnWarningColor(side);
        }
        smallfont_draw_string(2, 1, "OFF", color, false);
      } else {
        if (microLinnIsLinnCC(Split[side].microLinn.groupingCC)) {
          color = microLinnWarningColor(side);
        }
        paintNumericDataDisplay(color, Split[side].microLinn.groupingCC, isLinn200() ? 0 : 2, !isLinn200());
      }
      break;
  }
}
...
void microLinnHandlePerSplitXenSettingsNewTouch() {
  ...
  switch (microLinnPerSplitXenSettingsRowNum) {
    ...
    case 5: 
      ...
      break;
    case 4: 
      CC = Split[side].microLinn.groupingCC;
      if (CC == 255) CC = -1;
      handleNumericDataNewTouchCol(CC, -1, 127, true);
      Split[side].microLinn.groupingCC = CC == -1 ? 255 : CC;              // 255 or -1 means OFF
      updateDisplay();
      if (microLinnPerSplitXenSettingsLinked[4])
        Split[otherSide].microLinn.groupingCC = Split[side].microLinn.groupingCC;
      break;
  }
  updateDisplay(); 
}
...
void microLinnSetUpRechannelling(byte side) {
  ...
  if (microLinnNumGroups[side] >= 4) Split[side].midiChanMainEnabled = false;
...
// paint the 16 launching buttons in reverse order, to match the bottom-to-top order of ccForFader
void paintMicroLinnClipLauncher() {
  byte col, row;
  for (byte side = 0; side < NUMSPLITS; ++side) {
    for (byte i = 0; i < NUMROWS; ++i) {
      ...
      // the big PC number uses both colorMain and colorAccent, so linn128 needs a 3rd color
      byte color = isLinn200() ? Split[side].colorMain : Split[side].colorLowRow;
      if (8 * side + i == microLinnLastClipLaunched - 1) color = Split[side].colorAccent;
      setLed(col, row, color, cellOn);
    }
  }
}
...
void sendMicroLinnGroupingAndLocatingCCs(byte channel, byte side) {
  ...
  if (Split[side].microLinn.tuningTable == 2 && Split[side].microLinn.groupingCC <= 127) {
    midiSendControlChange(Split[side].microLinn.groupingCC, sensorCell->microLinnGroup + 1, channel, true); 
  }
  ...
  if (sensorCol <= 16 && Global.microLinn.locatingCC1 <= 127)
    midiSendControlChange(Global.microLinn.locatingCC1, (7 - sensorRow) + 8 * (sensorCol - 1),  channel, true);
  if (sensorCol >= 17 && Global.microLinn.locatingCC2 <= 127)
    midiSendControlChange(Global.microLinn.locatingCC2, (7 - sensorRow) + 8 * (sensorCol - 17), channel, true);
}
...
void receivedMicroLinnNrpn(int parameter, int value) {
  ...
  switch (parameter) {
    ...
    case 1003: if (inRange(value, 0, 3))   Split[side].microLinn.setHammerOnMode(value); break;
...
void microLinnImportGlobal(byte data1, byte data2, unsigned short i, byte presetNum) {     // version 72.1 size is 307 + 1 byte padding
  ...
  if (i == 0) {
    ...
    signed char n = (signed char)data2;
    if (microLinnInRange(n, -1, 127))   g->microLinn.locatingCC1 = data2;
  } else if (i == 168) {
    signed char n = isLinn200() ? (signed char)data1 : -1;
    if (microLinnInRange(n, -1, 127))   g->microLinn.locatingCC2 = data1;
    if (data2 >= 5) {
...
inline boolean isSkipFrettingOLD (byte side) {                          
...
// use locating CCs to light up the exact cell, better done with channelPerRow mode
boolean highlightExactNoteCell(byte split, byte notenum, byte channel) {
  if (userFirmwareActive) return false;
  if (displayMode != displayNormal) return false;
  if (microLinnHighlightExactNoteCell(split, notenum, channel, Global.microLinn.locatingCC1, 1))  return true;
  if (microLinnHighlightExactNoteCell(split, notenum, channel, Global.microLinn.locatingCC2, 17)) return true;
  if (Split[split].midiMode != channelPerRow) return false;
  (etc.)

boolean microLinnHighlightExactNoteCell(byte split, byte notenum, byte channel, byte CCtype, byte startCol) {
  if (CCtype <= 127 && calcTimeDelta(millis(), lastMomentMidiCC[channel][CCtype]) <= 2) {
    byte CCval = lastValueMidiCC[channel][CCtype];
    byte col = (CCval >> 3) + startCol;
    byte row = 8 - (CCval & B00000111);
    short locatedNote;
    if (isMicroLinnOn()) locatedNote = microLinnEdostep[split][col][row];
    //else locatedNote = 
    if (notenum == locatedNote) {
