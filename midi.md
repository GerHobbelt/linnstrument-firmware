LinnStrument sends the following MIDI output
============================================

```
Normal playing      New touch               Note On, XYZ data (microLinn adds an optional locating and/or grouping CC)
                    Release                 Note Off (Reset On Release adds a Pitch Bend of 0)
                    X movement              Pitch Bend
                    Y movement              Control Change (or Polyphonic Pressure or Channel Pressure)
                    Z movement              Polyphonic Pressure or Channel Pressure or Control Change

Low Row playing     LowRow = Sustain        X movement = Control Change (CC64 = Sustain)   
                    LowRow = Bend           X movement = Pitch Bend
                    LowRow = X=CC1          X movement = Control Change (or Channel Pressure)
                    LowRow = XYZ=16-18      XYZ movement = 3 types of Control Change (or Channel Pressure) (microLinn adds up to 3 more)
...
Preset display      Change the preset       Program Change (microLinn adds CC0 = Bank Select)
                    Launch a clip           Control Change

Volume display      X movement              Control Change (CC7 = Volume)

Global display      Notes Off               All Sound Off, All Notes Off, CC64 Sustain off, multiple NoteOffs

receive NRPN 299    NRPN value < 2048       export a 14-bit value via a NRPN 
                    NRPN value > 2048       microLinn: bulk export via a NRPN 300 header and multiple PolyPressure messages
```
...
1010cccc   0nnnnnnn   Polyphonic Pressure     microLinn: when preceeded by a NRPN 300 header, do a bulk import, see below
           0vvvvvvv                           c is the channel, n and v are data bytes, the 8th bit is encoded in the channel
```
...
| CC Number     | Used for
| --------------|--------------------------------------------------------------------------------------------------
| 0             | MicroLinn: Set the active bank for a split, the split is determined by its active MIDI mode channels
| 1-8 (default) | Change the position of the CC faders, the split is determined by its active MIDI mode channels
| 6             | Data entry for RPNs and NRPNs, but if CC6 occurs outside of an RPN/NRPN, it controls a CC fader instead
...
This is an overview of the NRPN message chain:
...
| RPN  | MSB Value | Description
|------|-----------|-------------------------------------------------------------------------------------------
| 0    | 1-96      | Set pitch bend range for both splits to 1-96 semitones
| 6    | 0-15      | Set up MPE on one split (channel 0 = L, 15 = R) for 1-15 channels, or if value is 0, turn off MPE
...
| 1     | 0-16  | Split Left MIDI Main Channel (0 = disable the main channel / report that the main channel is disabled)
...
| 50    | 0-2   | Split Left LowRow XYZ Behavior (0: Hold, 1: Fader, 2: Joystick)
...
| 61    | 0-14  | Split Left Touch Animation (0: Cell, 1: Same, 2: Blink, 3: Crosses, 4: Circles, 5: Squares, 6: Diamonds, 7: Stars, 8: Sparkles, 9: Curtains, 10: Blinds, 11: Targets, 12: Up, 13: Down, 14: Left, 15: Right, 16: Orbits)
...
| 150   | 0-2   | Split Right LowRow XYZ Behavior (0: Hold, 1: Fader, 2: Joystick)
...
| 161   | 0-14  | Split Right Touch Animation (0: Cell, 1: Same, 2: Blink, 3: Crosses, 4: Circles, 5: Squares, 6: Diamonds, 7: Stars, 8: Sparkles, 9: Curtains, 10: Blinds, 11: Targets, 12: Up, 13: Down, 14: Left, 15: Right, 16: Orbits)
...
| 203   | 0-1   | Global Main Note Light C (0: Off, 1: On)     NRPNs 203-226 overwrite the currently selected scale...
| 204   | 0-1   | Global Main Note Light C# (0: Off, 1: On)    ...if user is in a custom light pattern, nothing happens
...
| 236   | 1-7   | Global Arp Tempo Note Value (0:4, 1: 8, 2: 8 Tri, 3: 16, 4: 16 Swing, 5: 16 Tri, 6: 32, 7: 32 Tri)
...
| 243 * | 0-5   | Global Settings Preset (Memory) Load
...
|     * | 0-50  | ...or if an edo has been chosen, (0..50: -25..25 edosteps)
